...
namespace GloriousGuns.Items.Bandit.Unique
{
	public class KillingWord : ModItem
	{
		public static string[] RandNames = { "Dayumned","Say Krud","Shoty","Eezy","Broosin", "Hurrty", "Moar" };
		protected ushort nameIndex;
		//protected int counter;

		public string WeaponName =>  RandNames[nameIndex%RandNames.Length]+" Killing Word";
		...
		public override void SetStaticDefaults()
		{
            ...
            GloriousGunsGlowmask.AddGlowMask(item.type, "GloriousGuns/Items/Bandit/Unique/KillingWord_Glow");
		}
		public override void SetDefaults()
		{
			...
			item.rare = 3;
			item.autoReuse = true;
        	item.UseSound = mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/KillingWord");
			item.shoot = 10;
		...
		{
			var myClone = (KillingWord)base.Clone(itemClone);
		...
		{	
            ...
            type = mod.ProjectileType("KillingWordProj");
            
            {
                Vector2 muzzleOffset = Vector2.Normalize(new Vector2(speedX, speedY)) * 38f;
				...
				for (int index1 = 0; index1 < 5; ++index1)
                {
                    int index2 = Dust.NewDust(position, item.width - 100, item.height - 40, 68, 0.0f, 0.0f, (int) byte.MaxValue, new Color(), (float) GloriousGuns.instance.gloriousRNG.Next(20, 26) * 0.1f);
                    Main.dust[index2].noLight = true;
                    Main.dust[index2].noGravity = true;
                    Main.dust[index2].velocity *= 0.75f;
                    Main.dust[index2].scale *= .4f;
                }	
                magazine--;
		...
		{
			Lighting.AddLight(item.position,0.045f,0.220f,0.5f);
			Texture2D texture = Main.itemTexture[item.type];

			spriteBatch.Draw(Main.extraTexture[60],item.position-Main.screenPosition+item.Size*0.5f+new Vector2(-36,-46f),null,new Color(21,110,224,0),0f,texture.Size() * 0.5f,scale,SpriteEffects.None,0f);
		...
		public override void PostDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, float rotation, float  scale, int whoAmI) 	
		{
			...
			spriteBatch.Draw
			(
				ModContent.GetTexture("GloriousGuns/Items/Bandit/Unique/KillingWord_Glow"),
				new Vector2
				(
		...
		public void Generate()
		{
			...
			maxBullets = GloriousGuns.instance.gloriousRNG.Next(51, 68);
			BurstFireRate = item.useTime = GloriousGuns.instance.gloriousRNG.Next(24, 42);

			item.useAnimation = item.useTime = GloriousGuns.instance.gloriousRNG.Next(13, 16);
			item.damage =  GloriousGuns.instance.gloriousRNG.Next(14,17);
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
		{
		    ...
		    TooltipLine line22 = new TooltipLine(mod, "Damage", "His word will carry death eternal.");
            line22.overrideColor = new Color(204, 55, 55);
            tooltips.Add(line22);
