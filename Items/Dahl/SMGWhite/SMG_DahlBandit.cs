...
{
	public class SMG_DahlBandit : ModItem
	{
		...
		public string WeaponName => RandNames[nameIndex%RandNames.Length]+" SMG";
		...
		{
			var myClone = (SMG_DahlBandit)base.Clone(itemClone);
		...
		public void Generate()
		{
			...
			item.useAnimation = item.useTime = new UnifiedRandom().Next(9, 11);
			item.damage =  new UnifiedRandom().Next(4,6);
			item.knockBack =  new UnifiedRandom().Next(0,1);
			item.value = new UnifiedRandom().Next(11000,15000);
			SMGBulletsMax = new UnifiedRandom().Next(60, 90);
