...
namespace GloriousGuns.Items.Torgue.PistolGreen
{
	public class PocketRocket : ModItem
	{
		...
		{
			var myClone = (PocketRocket)base.Clone(itemClone);
		...
		{	
            ...
            }        
            {
				...
				for (int index1 = 0; index1 < 5; ++index1)
                {
					int index2 = Dust.NewDust(position, item.width - 66, item.height - 20, 127, 0.0f, 0.0f, (int) byte.MaxValue, new Color(), (float) GloriousGuns.instance.gloriousRNG.Next(20, 26) * 0.1f);
                    Main.dust[index2].noLight = true;
                    Main.dust[index2].noGravity = true;
                    Main.dust[index2].velocity *= 0.5f;
                    Main.dust[index2].scale *= .6f;
		...
		public void Generate()
		{
			...
			item.damage =  GloriousGuns.instance.gloriousRNG.Next(18,21);
