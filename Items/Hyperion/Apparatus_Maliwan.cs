...
{
	public class Apparatus_Maliwan : ModItem
	{
		...
		{
			var myClone = (Apparatus_Maliwan)base.Clone(itemClone);
		...
		{
			accurateBullets++;
			Vector2 muzzleOffset = Vector2.Normalize(new Vector2(speedX, speedY - 1)) * 30f;
			if (Collision.CanHit(position, 0, 0, position + muzzleOffset, 0, 0))
			{
				position += muzzleOffset;
			}
			Main.PlaySound(new Terraria.Audio.LegacySoundStyle(2,41));
			float spread = MathHelper.ToRadians(37f/accurateBullets); //45 degrees converted to radians
			float baseSpeed = (float)Math.Sqrt(speedX * speedX + speedY * speedY);
			double baseAngle = Math.Atan2(speedX,speedY);
			double randomAngle = baseAngle + (Main.rand.NextFloat() - 0.5f) * spread;
			speedX = baseSpeed * (float)Math.Sin(randomAngle);
			speedY = baseSpeed * (float)Math.Cos(randomAngle);
			if (accurateBullets >= 40)
			{
		...
		public void Generate()
		{
            ...
            item.crit = GloriousGuns.instance.gloriousRNG.Next(2,4);
