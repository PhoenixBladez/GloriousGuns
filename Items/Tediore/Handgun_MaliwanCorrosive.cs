...
{
	public class Handgun_MaliwanCorrosive : ModItem
	{
		...
		public string WeaponName => "Pine Fresh " + RandNames[nameIndex%RandNames.Length]+" Handgun";
		...
		{
			var myClone = (Handgun_MaliwanCorrosive)base.Clone(itemClone);
		...
		public override bool Shoot(Player player,ref Vector2 position,ref float speedX,ref float speedY,ref int type,ref int damage,ref float knockBack)
		{
			...
			if (handgunBullets == 1)
			{
				...
				type = mod.ProjectileType("Handgun_MaliwanCorrosiveProj");
        ...
        public override void PostDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, float rotation, float  scale, int whoAmI) 	
		{
			...
			spriteBatch.Draw
			(
				ModContent.GetTexture("GloriousGuns/Items/Tediore/Handgun_MaliwanCorrosiveGlow"),
				new Vector2
				(
		...
		public void Generate()
		{
			...
			item.useAnimation = item.useTime = new UnifiedRandom().Next(6, 10);
			item.damage =  new UnifiedRandom().Next(8,11);
			item.knockBack =  new UnifiedRandom().Next(1, 2);
			item.value = new UnifiedRandom().Next(600, 1000);
            item.reuseDelay = new UnifiedRandom().Next(5,10);
			item.shootSpeed =  new UnifiedRandom().NextFloat(9f,12f);
